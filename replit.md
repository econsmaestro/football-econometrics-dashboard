...
- **2026-03-26**: Added cup/tournament penalty fallback data — 16 CSVs in `fallback_data/cups/` covering takers and goalkeepers for all 8 cup competitions (Champions League, Europa League, Conference League, FIFA World Cup, UEFA European Championship, AFC Asian Cup, Copa América, CONMEBOL Libertadores). `models.py:load_fallback_penalty_data()` maps all 8 cups; `load_alltime_taker_penalties` and `load_alltime_gk_penalties` in `app.py` now accept `league_name` kwarg and auto-serve cached data with a blue info banner when Transfermarkt is unreachable.
